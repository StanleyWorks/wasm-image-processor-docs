...
const onSubmit = () => {
  ...
  reader.onload = async () => {
    ...
    const result = crop(uint8Array, x.value, y.value, width.value, height.value);
    const file = new File([result], "cropped", { type: picture.value.type });
...
<template>
  <div class="max-w-4xl mx-auto p-6">
    <h1 class="text-3xl font-bold text-center mb-6 text-gray-800">Crop Demo</h1>
    <div class="bg-white shadow-lg rounded-lg p-6 mb-8">
      <form action="#" @submit.prevent="onSubmit">
        ...
        <div class="mb-5 grid grid-cols-2 gap-4">
          <div>
            <label for="x" class="block text-gray-700 font-medium mb-2">X</label>
            <input id="x" v-model="x" type="number" class="w-full border border-gray-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-primary" placeholder="0" />
          </div>
          <div>
            <label for="y" class="block text-gray-700 font-medium mb-2">Y</label>
            <input id="y" v-model="y" type="number" class="w-full border border-gray-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-primary" placeholder="0" />
          </div>
          <div>
            <label for="width" class="block text-gray-700 font-medium mb-2">Width</label>
            <input id="width" v-model="width" type="number" class="w-full border border-gray-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-primary" placeholder="100" />
          </div>
          <div>
            <label for="height" class="block text-gray-700 font-medium mb-2">Height</label>
            <input id="height" v-model="height" type="number" class="w-full border border-gray-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-primary" placeholder="100" />
          </div>
        </div>
        <button type="submit" class="w-full bg-primary hover:bg-primary text-white py-3 px-4 rounded-lg font-medium transition disabled:bg-gray-300 disabled:cursor-not-allowed flex justify-center items-center" :disabled="useIsLoading().value || !picture">
          ...
          {{ useIsLoading().value ? "Processing..." : "Crop" }}
        </button>
      </form>
    </div>
    <div v-if="previewUrl || processedImageUrl" class="mb-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div v-if="processedImageUrl" class="flex flex-col items-center">
          <div class="text-lg font-medium text-gray-700 mb-2">Cropped</div>
          <div class="relative border-2 border-green-500 rounded-lg overflow-hidden shadow-md bg-gray-100 p-2">
            <img class="max-h-64 max-w-full object-contain mx-auto" :src="processedImageUrl" alt="Cropped image" />
          </div>
