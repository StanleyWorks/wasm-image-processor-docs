...
<template>
  <div class="max-w-4xl mx-auto p-6">
    <h1 class="text-3xl font-bold text-center mb-6 text-gray-800">
      WebAssembly Resize Square Demo
    </h1>
    <div class="bg-white shadow-lg rounded-lg p-6 mb-8">
      <form action="#" @submit.prevent="onSubmit">
        ...
        <div class="mb-5">
          <label for="side" class="block text-gray-700 font-medium mb-2">Target size (pixels)</label>
          <div class="flex items-center">
            <input id="side" class="w-full border border-gray-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-primary" v-model="side" min="1" placeholder="512" type="number" />
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div v-if="resizedImageUrl" class="flex flex-col items-center">
          ...
          <div class="relative border-2 border-green-500 rounded-lg overflow-hidden shadow-md bg-gray-100 p-2">
            <img class="max-h-64 max-w-full object-contain mx-auto" :src="resizedImageUrl" alt="Resized image" />
          </div>
          <div v-if="resizedImage" class="mt-2 text-sm text-gray-500">Size: {{ useFileSize(resizedImage) }}</div>
          <div>
            <a v-if="resizedImage" download :href="useCreateObjectUrl(resizedImage)">Download</a>
          </div>
        </div>
      </div>
    </div>
